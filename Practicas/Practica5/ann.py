...
def train_neural_network(X,y,input_size,hidden_size,output_size,lambda_,alpha,num_iterations):
    m=len(X)
    # One-Hot Encoding
    # Inicializar el codificador
    encoder = OneHotEncoder(sparse=False)

    # Reshape y aplicar one-hot encoding
    ...
    for i in range(num_iterations):
        ...
        cost_func = lambda p: backdrop(np.reshape(p[:hidden_size * (input_size + 1)], (hidden_size, input_size + 1)),
                                       np.reshape(p[hidden_size * (input_size + 1):], (output_size, hidden_size + 1)),
                                       X, y_onehot, lambda_)[0]

        grad_func = lambda p: np.concatenate((backdrop(np.reshape(p[:hidden_size * (input_size + 1)], (hidden_size, input_size + 1)),
                                                       np.reshape(p[hidden_size * (input_size + 1):], (output_size, hidden_size + 1)),
                                                       X, y_onehot, lambda_)[1].flatten(),
                                              backdrop(np.reshape(p[:hidden_size * (input_size + 1)], (hidden_size, input_size + 1)),
                                                       np.reshape(p[hidden_size * (input_size + 1):], (output_size, hidden_size + 1)),
